...
export default function AddEditColorModal({ open, onClose, colorId, onSuccess }) {
  ...
  const { data: colorData, isLoading: isLoadingColor } = useApiRequest({
  ...
  const onSubmit = (data) => {
    ...
    saveColor(data, {
      ...
      onError: (error) => {
        ...
        showError(`Failed to ${isEditing ? "update" : "create"} color: ${error.response?.data?.message || error.message}`);
      }
    });
  };
            ...
            <Box sx={{ pt: 1, pb: 1 }}>
              ...
              {isEditing && (
                <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 2 }}>
                  Last Updated: {colorData?.tanggal_update ? new Date(colorData.tanggal_update).toLocaleString() : "N/A"}
                </Typography>
              )}
            </Box>
          )}
